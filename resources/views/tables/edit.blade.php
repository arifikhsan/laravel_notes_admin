            ...
            <div class="mt-10 sm:mt-0">
              <div class="">
                <div class="mt-5 md:mt-0 md:col-span-2">
                  <form action="/secretroom/{{ $table }}/{{ $id }}/update" method="POST">
                    @method('PUT')
                    @csrf
                            ...
                            <div class="col-span-6">
                              ...
                              @if ($column->name == 'id')
                                <input type="number"
                                       id="{{ $column->name }}"
                                       value="{{ $item->$value }}"
                                       class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-md disabled:opacity-50"
                                       disabled>
                                <input type="hidden"
                                       name="{{ $column->name }}"
                                       value="{{ $item->$value }}">
                              @elseif ($column->type == 'integer' || $column->type == 'bigint')
                                <input type="number"
                                       name="{{ $column->name }}"
                                       id="{{ $column->name }}"
                                       value="{{ $item->$value }}"
                              ...
                              @elseif($column->type == 'text')
                                <textarea
                                  ...
                                  placeholder="lorem ipsum">{{ $item->$value }}</textarea>
                              @else
                                <input type="text"
                                       name="{{ $column->name }}"
                                       id="{{ $column->name }}"
